...
def build_export_pipeline(client_config, checkpoint_service, args) -> Pipeline:
    ...
    export_service_principals = pipeline.add_task(ServicePrincipalExportTask(client_config, checkpoint_service, wmconstants.SERVICE_PRINCIPALS in skip_tasks), [export_instance_profiles])
    export_groups = pipeline.add_task(GroupExportTask(client_config, checkpoint_service, wmconstants.GROUPS in skip_tasks), [export_users, export_service_principals])
    ...
    export_workspace_dirs = pipeline.add_task(WorkspaceDirectoryExportTask(client_config, checkpoint_service, wmconstants.WORKSPACE_DIRECTORY_OBJECT in skip_tasks), [workspace_item_log_export])
    export_notebooks = pipeline.add_task(NotebookExportTask(client_config, checkpoint_service, wmconstants.NOTEBOOKS in skip_tasks), [export_workspace_dirs])
...
def build_import_pipeline(client_config, checkpoint_service, args) -> Pipeline:
    ...
    import_service_principals = pipeline.add_task(ServicePrincipalImportTask(client_config, checkpoint_service, wmconstants.SERVICE_PRINCIPALS in skip_tasks), [import_instance_profiles])
    import_groups = pipeline.add_task(GroupImportTask(client_config, checkpoint_service, wmconstants.GROUPS in skip_tasks), [import_users, import_service_principals])
