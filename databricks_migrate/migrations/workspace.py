...
class WorkspaceMigrations(BaseMigrationClient):
    ...
    def export_user_home(self, username, local_export_dir):
        ...
        log.info("Export path: {0}".format(self._export_dir))
        ...
        if items is not None:
            ...
            with open(workspace_log, "a") as ws_fp, open(libs_log, "a") as libs_fp:
                for x in notebooks:
                    ...
                    log.debug("Saving path: {0}".format(x))
                    ws_fp.write(x + '\n')
                for y in libraries:
                    libs_fp.write(y + '\n')
    ...
    def import_all_workspace_items(self, load_dir='logs/', export_dir='artifacts/', archive_missing=False):
        ...
        if num_current_users == 0:
            log.error("No registered users in existing environment. Please import users / groups first.")
        ...
        if (num_current_users < num_exported_users) and (not archive_missing):
            log.info("Exported number of user workspaces: {0}".format(num_exported_users))
            log.info("Current number of user workspaces: {0}".format(num_current_users))
            log.info("Re-run with the archive flag to load missing users into a separate directory")
            ...
            if self.is_user_ws_item(upload_dir):
                ...
                if archive_missing:
                    if ws_user in archive_users:
                        ...
                        log.info("User workspace does not exist, adding to archive cache: {0}".format(ws_user))
                        archive_users.add(ws_user)
                        # append the archive path to the upload directory
                        upload_dir = upload_dir.replace('Users', 'Archive', 1)
                    else:
                        log.info("User workspace exists: {0}".format(ws_user))
                elif not self.does_user_exist(ws_user):
                    log.error("User {0} is missing. "
                          "Please re-run with --archive-missing flag "
                          "or first verify all users exist in the new workspace".format(ws_user))
                    return
                else:
                    log.info("Uploading for user: {0}".format(ws_user))
